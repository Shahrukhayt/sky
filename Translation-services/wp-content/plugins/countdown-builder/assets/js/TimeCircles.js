        ...
        // Check if this is an unpause of a timer
            ...
            if (typeof attr_data_timer === "number") {
                ...
                if(this.config['ycd-count-up-from-end-date'] && this.config['countdownExpired']) {
                    this.data.attributes.ref_date = new Date(ycdmoment(this.config['ycd-date-time-picker']).tz(this.config['ycd-time-zone']).format('MM/DD/YYYY H:m:s')).getTime() + (attr_data_timer * 1000);
                }
            }
            else {
