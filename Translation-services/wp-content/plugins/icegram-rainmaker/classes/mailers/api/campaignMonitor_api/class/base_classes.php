...
defined( 'Rm_REST_WRAPPER_VERSION' ) or define( 'Rm_REST_WRAPPER_VERSION', '5.0.2' );
defined( 'Rm_HOST' ) or define( 'Rm_HOST', 'api.createsend.com' );
defined( 'Rm_OAUTH_BASE_URI' ) or define( 'Rm_OAUTH_BASE_URI', 'https://' . Rm_HOST . '/oauth' );
defined( 'Rm_OAUTH_TOKEN_URI' ) or define( 'Rm_OAUTH_TOKEN_URI', Rm_OAUTH_BASE_URI . '/token' );
defined( 'Rm_REST_WEBHOOK_FORMAT_JSON' ) or define( 'Rm_REST_WEBHOOK_FORMAT_JSON', 'json' );
defined( 'Rm_REST_WEBHOOK_FORMAT_XML' ) or define( 'Rm_REST_WEBHOOK_FORMAT_XML', 'xml' );
 ...
 *
 */
if ( ! class_exists( 'Rm_REST_Wrapper_Result' ) ) {
	class Rm_REST_Wrapper_Result {
		/**
 ...
 *
 */
if ( ! class_exists( 'Rm_REST_Wrapper_Base' ) ) {
	class Rm_REST_Wrapper_Base {
		/**
		 ...
		 * @var Rm_REST_JsonSerialiser or Rm_REST_XmlSerialiser
		 * @access private
		 */
		var $_serialiser;
		 ...
		 * @var Rm_REST_CurlTransport or Rm_REST_SocketTransport or your own custom transport.
		 * @access private
		 */
		var $_transport;
		 ...
		 * @param $debug_level int The level of debugging required Rm_REST_LOG_NONE | Rm_REST_LOG_ERROR | Rm_REST_LOG_WARNING | Rm_REST_LOG_VERBOSE
		 * @param $host string The host to send API requests to. There is no need to change this
		 * @param $log Rm_REST_Log The logger to use. Used for dependency injection
		...
		function __construct(
			...
			$this->_log = is_null( $log ) ? new Rm_REST_Log( $debug_level ) : $log;

			$this->_protocol   = $protocol;
			$this->_base_route = $protocol . '://' . $host . '/api/v3.1/';

			$this->_log->log_message( 'Creating wrapper for ' . $this->_base_route, get_class( $this ), Rm_REST_LOG_VERBOSE );

			$this->_transport = is_null( $transport ) ?
				Rm_REST_TRANSPORT_get_available( $this->is_secure(), $this->_log ) :
				$transport;

			$transport_type = method_exists( $this->_transport, 'get_type' ) ? $this->_transport->get_type() : 'Unknown';
			$this->_log->log_message( 'Using ' . $transport_type . ' for transport', get_class( $this ), Rm_REST_LOG_WARNING );

			$this->_serialiser = is_null( $serialiser ) ?
				Rm_REST_SERIALISATION_get_available( $this->_log ) : $serialiser;

			$this->_log->log_message( 'Using ' . $this->_serialiser->get_type() . ' json serialising', get_class( $this ), Rm_REST_LOG_WARNING );

			$this->_default_call_options = array(
				'authdetails' => $auth_details,
				'userAgent'   => 'Rm_REST_Wrapper v' . Rm_REST_WRAPPER_VERSION .
		...
		function refresh_token() {
			...
			$wrap    = new Rm_REST_Wrapper_Base(
				null, 'https', Rm_REST_LOG_NONE, Rm_HOST, null,
				new Rm_REST_DoNothingSerialiser(), null );

			$result = $wrap->post_request( Rm_OAUTH_TOKEN_URI, $body, $options );
		...
		function put_request( $route, $data, $call_options = array() ) {
			return $this->_call( $call_options, Rm_REST_PUT, $route, $data );
		}

		function post_request( $route, $data, $call_options = array() ) {
			return $this->_call( $call_options, Rm_REST_POST, $route, $data );
		}

		function delete_request( $route, $call_options = array() ) {
			return $this->_call( $call_options, Rm_REST_DELETE, $route );
		}

		function get_request( $route, $call_options = array() ) {
			return $this->_call( $call_options, Rm_REST_GET, $route );
		}
		...
		function _call( $call_options, $method, $route, $data = null ) {
			...
			$this->_log->log_message( 'Making ' . $call_options['method'] . ' call to: ' . $call_options['route'], get_class( $this ), Rm_REST_LOG_WARNING );
			...
			return new Rm_REST_Wrapper_Result( $call_result['response'], $call_result['code'] );
		}
	}
}